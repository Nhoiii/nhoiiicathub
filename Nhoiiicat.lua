if getgenv().Nousigi then 
	if game.CoreGui:FindFirstChild("Nousigi Hub GUI") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name,  "NhoiiiCatHub") then
				v:Destroy()
			end
		end
	end
...
local T1UIColor = {
	...
	["Logo Image"] = "rbxassetid://120185482338636",
	["Search Icon Color"] = Color3.fromRGB(200, 200, 200),
	["Search Icon Highlight Color"] = Color3.fromRGB(0, 120, 255),
	...
	["Title Text Color"] = Color3.fromRGB(120, 190, 255),
	...
	["Page Selected Color"] = Color3.fromRGB(0, 90, 200),
	["Section Text Color"] = Color3.fromRGB(200, 200, 200),
	["Section Underline Color"] = Color3.fromRGB(0, 120, 255),

	["Toggle Border Color"] = Color3.fromRGB(0, 120, 255),
	["Toggle Checked Color"] = Color3.fromRGB(0, 150, 255),
	...
	["Dropdown Selected Color"] = Color3.fromRGB(0, 90, 200),
	["Dropdown Selected Check Color"] = Color3.fromRGB(0, 60, 150),

	["Textbox Highlight Color"] = Color3.fromRGB(0, 120, 255),
	["Box Highlight Color"] = Color3.fromRGB(0, 120, 255),

	["Slider Line Color"] = Color3.fromRGB(0, 120, 255),
	["Slider Highlight Color"] = Color3.fromRGB(0, 60, 150),
...
btnHide.Image = getgenv().UIColor["https://cdn.discordapp.com/avatars/1212720902299590686/97ebe85264b1f3ff28e42ff5b848a1f9.png?size=4096"]
...
btnHideFrame.Name = "NhoiiiCatHub"
...
Library.ToggleUI = function()
	...
	if game.CoreGui:FindFirstChild("NhoiiiCatHub GUI") then
		for a, b in ipairs(game.CoreGui:GetChildren()) do
			if b.Name == "NhoiiiCatHub GUI" then
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title or "NhoiiiCat Hub"
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = tostring(TitleNameMain or "Nhoi Hub - Blox Fruit")
	...
	function Main_Function:AddTab(PageName)
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			function sectionFunction:AddToggle(idk,Setting)
				...
				ToggleTitle.Position = UDim2.new(0, 10, 0,)
...
local Window = Library:CreateWindow({
    Title = "Nhoi Hub Beta",
    SubTitle = "",
    SaveFolder = "Blox Fruts",
    Image = "rbxassetid://120185482338636"
})
...
task.delay(2, function()
	...
	Library:Notify({Title = "Nhoi Hub", Content = "Script carregado! Bem-vindo, " .. game.Players.LocalPlayer.Name, Icon = "star", Duration = 5});
end);

InfoTab:AddSection(" Nhoi Hub ");
InfoTab:AddParagraph({
	Title = "Nhoi Hub",
	Desc = "Version: R6.1 | Game: Blox Fruits\nMade for the Nhoi Hub Community\nAll features are free to use."
});
InfoTab:AddParagraph({
...
InfoTab:AddButton({
	Title = "Join Nhoi Community Discord",
	Desc = "Click to open the Discord invite link",
	Callback = function()
		setclipboard("https://discord.gg/aXWW8hytgX");
		Library:Notify({Title = "Nhoi Hub", Content = "Discord link copied to clipboard!\ndiscord.gg/f4K5sDwKkn", Icon = "bell", Duration = 5});
	end
});
InfoTab:AddParagraph({
	Title = " Credits",
	Desc = "Dev: MutanoZS\nOwner: KettyS\n\nNhoi Hub R6.1 - Blox Fruits"
});
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Nhoi Hub", Content = "Item especial encontrado! Auto Chest parado.", Icon = "bell", Duration = 6});
				return;
			end;
			...
			for _, entry in ipairs(chests) do
				...
				if _isSpecialChestItem() then
					_chestTweenActive = false;
					_G.Settings.Farm["Auto Farm Chest Tween"] = false;
					Library:Notify({Title = "Nhoi Hub", Content = "Item especial encontrado!", Icon = "bell", Duration = 6});
					break;
				end;
			...
			if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
				_G.ChestHopCount = 0;
				Library:Notify({Title = "Nhoi Hub", Content = "Chest Hop: limite atingido, trocando server...", Icon = "bell", Duration = 4});
				task.wait(2);
				local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
				module:Teleport(game.PlaceId);
			end;
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Nhoi Hub", Content = "Item especial encontrado! Chest Bypass parado.", Icon = "bell", Duration = 6});
				return;
			end;
		...
		if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
			_G.ChestHopCount = 0;
			Library:Notify({Title = "Nhoi Hub", Content = "Chest Hop: trocando server...", Icon = "bell", Duration = 4});
			task.wait(2);
			local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
			module:Teleport(game.PlaceId);
		end;
...
local function ShowResetConfirm()
	pcall(function()
		...
		simBtn.MouseButton1Click:Connect(function()
			...
			Library:Notify({Title = "Nhoi Hub", Content = "Settings resetados! Reexecute o script.", Icon = "bell", Duration = 6});
		end);
...
local function BuyFightStyleFull(styleName)
	...
	if not data then
		Library:Notify({Title = "Nhoi Hub", Content = "Estilo nao encontrado: " .. tostring(styleName), Icon = "alert-triangle", Duration = 4});
		return;
	end;
	local plr = game.Players.LocalPlayer;
	local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart");
	...
	Library:Notify({Title = "Nhoi Hub", Content = "Compra enviada: " .. styleName, Icon = "check", Duration = 4});
end;
					...
					if PortalIslands and PortalIslands[_G.SelectIsland] then
						...
						if target then
							...
							if hrp and (hrp.Position - destCF.p).Magnitude < 20 then
								-- Chegou! Para o loop
								_G.BypassTeleportActive = false;
								Library:Notify({Title = "Nhoi Hub", Content = "Chegou em: " .. _G.SelectIsland, Icon = "map-pin", Duration = 4});
							end;
						end;
					end;
				end);
			...
			if nightTime then
				for _, v in pairs(mystic:GetDescendants()) do
					if v:IsA("MeshPart") and v.Material == Enum.Material.Neon and v.BrickColor == BrickColor.new("Bright blue") then
						...
						task.wait(0.2);
						pcall(function()
							Library:Notify({Title = "Nhoi Hub", Content = "Quest completa!", Icon = "bell", Duration = 6});
						end);
